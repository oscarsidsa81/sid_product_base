...
class ProductTemplate(models.Model):
    ...
    sid_AXI = fields.Char(string="Referencia AXI", store=True, tracking=True, help="Referencia AXI para tener trazabilidad con el sistema anterior")
    ...
    sid_coste_medio = fields.Float(string="Coste Medio", compute="_compute_sid_coste_medio", tracking=True, readonly=True, help="Coste medio para poder realizar el tracking\n"
    ...
    def _compute_precio_ton(self):
        for record in self:
            record.precio_ton = round((record.standard_price/record.weight*1000),0) or 0.0
    ...
    @api.depends("product_variant_ids", "product_variant_ids.virtual_available")
    ...
    def _compute_purchased_qty_3y(self):
        ...
        date_field = "order_id.date_approve" if "date_approve" in self.env["purchase.order"]._fields else "order_id.date_order"
